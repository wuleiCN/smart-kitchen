...
<template>
  <div>
    <Selection :types="4" @selection="selection" @registOn="registOn" />
    <el-card>
      <el-table
        v-loading="loading"
        :data="tableData"
      ...
      >
        ...
        <el-table-column prop="brandName" label="摄像机品牌" width="120" align="center" />
        ...
        <el-table-column prop="RegistOn" label="人脸识别" align="center">
          <template #default="{row: Has}">
            <el-switch v-model="Has.FaceAI" disabled active-color="#13ce66" inactive-color="#ff4949" />
          </template>
        </el-table-column>
        <el-table-column prop="RegistOn" label="人体识别" align="center">
          <template #default="{row: Has}">
            <el-switch v-model="Has.HumanAI" disabled active-color="#13ce66" inactive-color="#ff4949" />
          </template>
        </el-table-column>
    ...
    <el-dialog title="添加" :visible.sync="dialogAddDataVisible" @close="addFormClose">
      <el-form ref="addFormRef" :model="addForm" :rules="rules">
        ...
        <el-form-item label="设备状态" label-width="120px" prop="Code">
          <Select :url="deviceStatus" :foo.sync="addForm.Status" />
        </el-form-item>
        ...
        <el-form-item label="IP地址" label-width="120px" prop="Code">
          <el-input v-model="addForm.IP" autocomplete="off" placeholder="格式：192.168.1.11" />
        </el-form-item>
        ...
        <el-form-item label="登录端口" label-width="120px" prop="Code">
          <el-input v-model="addForm.Port" autocomplete="off" />
        </el-form-item>
        <el-form-item label="NVR通道号" label-width="120px" prop="Code">
          <el-input v-model="addForm.Channel" autocomplete="off" />
        </el-form-item>
        <el-form-item label="人脸智能" label-width="120px">
          <el-switch v-model="addForm.FaceAI" active-color="#13ce66" inactive-color="#ff4949" />
        </el-form-item>
        <el-form-item label="人体识别" label-width="120px">
          <el-switch v-model="addForm.HumanAI" active-color="#13ce66" inactive-color="#ff4949" />
        </el-form-item>
        <el-form-item label="注册日期" label-width="120px">
          <span>{{ addForm.RegistOn | dateFormat }}</span>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    async getDeviceModelList() {
      try {
        ...
        this.devicesModel = res.filter(v => v.Type === 4)
    ...
    async getDeviesInfo() {
      try {
        const { data: res } = await getCamerasByCompany({
    ...
    async editFormEnter() {
      this.$refs.editFormRef.validate(async (valid) => {
        if (valid) {
          try {
            const { data: res } = await CamerasUpdate()
    ...
    async addFormEnter() {
      this.$refs.addFormRef.validate(async (valid) => {
        if (valid) {
          try {
            const { data: res } = await CamerasRegist(this.addForm)
